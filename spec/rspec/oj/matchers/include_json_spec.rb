...
RSpec.describe RSpec::Oj::Matchers::IncludeJson do
  ...
  it 'provides a useful failure message for should' do
    ...
    expect(matcher.failure_message).to eq('Expected [1,2,3] to include [4,5,6]')
  end

  it 'provides a useful failure message when negated' do
    matcher = include_json(%(3))
    matcher.matches?(%([1,2,3]))
    expect(matcher.failure_message_when_negated).to eq('Expected [1,2,3] to not include 3')
  end
